; !function (win)
{
     ...
     * @param {any} title 标题
     * @param {any} url 链接地址
     * @param {any} is_refresh 是否刷新
     * @param {any} id //ifarme id
     */
    Xadmin.prototype.add_tab = function (title, url, is_refresh, id)
    {
        var iid;
        if (id === undefined) {
            iid = md5(url);//md5每个url
        } else {
            iid = id;
        }
        ...
        for (var i = 0; i < $('.x-iframe').length; i++) {
            if ($('.x-iframe').eq(i).attr('tab-id') === iid) {
                element.tabChange('xbs_tab', iid);
                if (is_refresh)
        ...
        this.add_lay_tab(title, url, iid);
        this.set_data(title, url, iid);
        element.tabChange('xbs_tab', iid);
    };
